...
async function aiProxy(path,body,res){try{const response=await fetch(`${aiServiceUrl}${path}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body),signal:AbortSignal.timeout(30000)});const payload=await response.json().catch(()=>({}));if(!response.ok)return res.status(response.status===503?503:502).json({error:payload.detail||'Multilingual service is unavailable'});res.json(payload)}catch{return res.status(503).json({error:'Multilingual service is temporarily unavailable'})}}
...
app.get('/api/multilingual/health',async(_,res)=>{try{const response=await fetch(`${aiServiceUrl}/health`,{signal:AbortSignal.timeout(5000)});res.status(response.ok?200:503).json(await response.json())}catch{res.status(503).json({ready:false,error:'Multilingual service is temporarily unavailable'})}})
app.post('/api/translate',(req,res)=>{const{text,sourceLanguage,targetLanguage}=req.body||{};if(typeof text!=='string'||!text.trim()||text.length>4000)return res.status(400).json({error:'Text must be between 1 and 4000 characters'});if(!supportedLanguages.has(sourceLanguage)||!supportedLanguages.has(targetLanguage))return res.status(400).json({error:'Unsupported language'});return aiProxy('/translate',{text:text.trim(),sourceLanguage,targetLanguage},res)})
app.post('/api/tts',(req,res)=>{const{text,language}=req.body||{};if(typeof text!=='string'||!text.trim()||text.length>4000)return res.status(400).json({error:'Text must be between 1 and 4000 characters'});if(!['te','hi'].includes(language))return res.status(400).json({error:'Voice generation supports Telugu and Hindi'});return aiProxy('/tts',{text:text.trim(),language},res)})
app.get('/api/tts/audio/:name',async(req,res)=>{if(!/^[a-f0-9]{64}\.wav$/.test(req.params.name))return res.status(400).json({error:'Invalid audio name'});try{const response=await fetch(`${aiServiceUrl}/audio/${req.params.name}`,{signal:AbortSignal.timeout(30000)});if(!response.ok)return res.status(response.status).end();res.setHeader('content-type','audio/wav');res.send(Buffer.from(await response.arrayBuffer()))}catch{res.status(503).json({error:'Voice audio is unavailable'})}})
