...
export default function MapView() {
  ...
  return <div className="min-h-[calc(100vh-8rem)] space-y-4 pb-8">
    ...
    <div className="relative h-[58vh] min-h-[420px] rounded-xl overflow-hidden border border-subtle"><div ref={container} className="absolute inset-0 bg-sidebar" />{mode && <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[500] max-w-[calc(100%-2rem)] bg-card border border-subtle p-3 rounded-xl shadow-xl flex flex-wrap justify-center items-center gap-2"><span className="text-sm flex items-center gap-2 whitespace-nowrap"><MapPin size={16} />Click map: {points.length} points</span><Button size="sm" variant="ghost" disabled={!points.length} onClick={() => setPoints((current) => current.slice(0, -1))}>Undo point</Button><Button size="sm" variant="ghost" disabled={!points.length} onClick={() => setPoints([])}>Clear</Button><Button size="sm" variant="ghost" onClick={cancel}><X size={16} />Cancel</Button><Button size="sm" disabled={points.length < 3} onClick={complete}><Check size={16} />Continue</Button></div>}</div>
